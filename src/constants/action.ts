export const LOGIN_REQUEST = 'LOGIN_REQUEST'
export const LOGIN_SUCCESS = 'LOGIN_SUCCESS'
export const LOGOUT_SUCCESS = 'LOGOUT_SUCCESS'
export const USER_AUTH_SUCCESS = 'USER_AUTH_SUCCESS'

export const LOAD_WORKSPACE_SUCCESS = 'LOAD_WORKSPACE_SUCCESS'
export const LOAD_WORKSPACE_REQUEST = 'LOAD_WORKSPACE_REQUEST'

export const LOAD_PROJECTS_REQUEST = 'LOAD_PROJECTS_REQUEST'
export const LOAD_PROJECTS_SUCCESS = 'LOAD_PROJECTS_SUCCESS'
export const SELECT_PROJECT = 'SELECT_PROJECT'
export const UPDATE_PROJECT_SUCCESS = 'UPDATE_PROJECT_SUCCESS'
export const CREATE_PROJECT_SUCCESS = 'CREATE_PROJECT_SUCCESS'
export const LOAD_PROJECT_COMMENTS_SUCCESS = 'LOAD_PROJECT_COMMENTS_SUCCESS'
export const PROJECT_NEW_COMMENT = 'PROJECT_NEW_COMMENT'

export const LOAD_ORGANIZATIONS_REQUEST = 'LOAD_ORGANIZATIONS_REQUEST'
export const LOAD_ORGANIZATIONS_SUCCESS = 'LOAD_ORGANIZATIONS_SUCCESS'
export const SELECT_ORGANIZATION = 'SELECT_ORGANIZATION'
export const UPDATE_ORGANIZATION_SUCCESS = 'UPDATE_ORGANIZATION_SUCCESS'
export const CREATE_ORGANIZATION_SUCCESS = 'CREATE_ORGANIZATION_SUCCESS'

export const SOCKET_CONNECT = 'CONNECT_SOCKET'
export const SOCKET_CONNECTED = 'SOCKET_CONNECTED'
export const SOCKET_CONNECTING = 'SOCKET_CONNECTING'
export const CONNECT_SOCKET_FAILURE = 'CONNECT_SOCKET_FAILURE'

export const CHANNEL_CONNECTED = 'CHANNEL_CONNECTED'
export const CHANNEL_CONNECTING = 'CHANNEL_CONNECTING'

export const LOAD_USER_NOTIFICATIONS = 'LOAD_USER_NOTIFICATIONS'

export const LOAD_ISSUES_REQUEST = 'LOAD_ISSUES_REQUEST'
export const LOAD_ISSUES_SUCCESS = 'LOAD_ISSUES_SUCCESS'
export const LOAD_MORE_ISSUES_SUCCESS = 'LOAD_MORE_ISSUES_SUCCESS'
export const SELECT_ISSUE = 'SELECT_ISSUE'
export const UPDATE_ISSUE_SUCCESS = 'UPDATE_ISSUE_SUCCESS'
export const LOAD_ISSUES_FAILED = 'LOAD_ISSUES_FAILED'

export const LOAD_TASK_REQUEST = 'LOAD_TASK_REQUEST'
export const LOAD_TASK_SUCCESS = 'LOAD_TASK_SUCCESS'
export const UPDATE_TASK_SUCCESS = 'UPDATE_TASK_SUCCESS'
export const CREATE_TASK_SUCCESS = 'CREATE_TASK_SUCCESS'

export const LOAD_CHANNELS_REQUEST = 'LOAD_CHANNELS_REQUEST'
export const LOAD_CHANNELS_SUCCESS = 'LOAD_CHANNELS_SUCCESS'
export const SELECT_CHANNEL = 'SELECT_CHANNEL'
export const LOAD_CHANNEL_MESSAGES_REQUEST = 'LOAD_CHANNEL_MESSAGES_REQUEST'
export const LOAD_CHANNEL_MESSAGES_SUCCESS = 'LOAD_CHANNEL_MESSAGES_SUCCESS'
export const CHANNEL_NEW_MESSAGE = 'CHANNEL_NEW_MESSAGE'
